...
void DepthTesting::OnUpdate(float t) {
...
void DepthTesting::CreateRenderPass() {
  ...
  if (const auto format =
    ...
    BOOST_ASSERT_MSG(false, "Failed to find suitable depth format!");
  }
...
void DepthTesting::CreateDescriptorSetLayouts() {
...
void DepthTesting::DestroyDescriptorSetLayouts() {
  vkDestroyDescriptorSetLayout(instance_.device, descriptors_.layout, nullptr);
}

void DepthTesting::CreatePipelines() {
  {
...
void DepthTesting::DestroyPipelines() { pipelines_.Destroy(instance_); }

void DepthTesting::CreateDepthStencil() {
  depth_.Create(instance_, swapchain_);
}

void DepthTesting::DestroyDepthStencil() { depth_.Destroy(instance_); }

void DepthTesting::CreateFramebuffers() {
  framebuffers_.resize(swapchain_.views.size());
  for (size_t i = 0; i < framebuffers_.size(); i++) {
    std::vector<VkImageView> attachments = {swapchain_.views[i], depth_.view};
...
void DepthTesting::CreateIndexBuffer() {
  index_.Create(instance_, indices, VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
}

void DepthTesting::CreateDrawCommandBuffers() {
...
void DepthTesting::CreateUniformBuffers() {
...
void DepthTesting::DestroyUniformBuffers() {
  for (size_t i = 0; i < swapchain_.images.size(); i++) {
    uniforms_[i].Destroy(instance_);
  }
}

void DepthTesting::UpdateUniformBuffers(uint32_t imageIndex) {
...
void DepthTesting::CreateDescriptorPool() {
...
void DepthTesting::CreateDescriptorSets() {
